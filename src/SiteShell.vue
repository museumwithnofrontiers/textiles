...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('textiles.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('textiles.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('textiles.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
